...
class NotificationScreenA extends StatefulWidget {
  ...
  final Key? key; // <-- thêm key để widget cha có thể reset
  const NotificationScreenA({this.key, this.onTapNotification})
    : super(key: key);

  @override
  State<NotificationScreenA> createState() => NotificationScreenAState();
}

class NotificationScreenAState extends State<NotificationScreenA> {
  List allNotifications = [];
  List unviewedNotifications = [];
  int currentIndex = 0;
  String? error;
  Timer? timer; // <-- đảm bảo khai báo Timer? timer ở đây
  ...
  @override
  void dispose() {
    timer?.cancel(); // <-- đảm bảo sử dụng đúng biến timer
    super.dispose();
  }
