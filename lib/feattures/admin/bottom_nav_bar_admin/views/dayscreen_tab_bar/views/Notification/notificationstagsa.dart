...
class NotificationsTagsA extends StatefulWidget {
  ...
  final VoidCallback? onRefreshNotifications; // <-- thêm callback
  const NotificationsTagsA({
    super.key,
...
class _NotificationsTagsAState extends State<NotificationsTagsA> {
  ...
  Future<void> handleAccept() async {
    ...
    try {
      final response = await http.put(
        Uri.parse('https://backendapperss.onrender.com/admin_accept_request'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'id': selectedNotification!['id']}),
      );
      if (response.statusCode == 200) {
        ...
        widget.onRefreshNotifications?.call(); // <-- gọi callback cập nhật
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Yêu cầu đã được chấp nhận.')),
          );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Đã xảy ra lỗi khi chấp nhận yêu cầu.')),
        );
      }
    }
  }
  ...
  Future<void> confirmReject() async {
    if (selectedNotification == null || rejectionReason.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      final response = await http.put(
        Uri.parse('https://backendapperss.onrender.com/admin_reject_request'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Yêu cầu đã bị từ chối.')),
          );
          Navigator.of(context).pop();
        }
      } else {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Đã xảy ra lỗi khi từ chối yêu cầu.')),
        );
      }
    }
  }
            ...
            if (isModalVisible && selectedNotification != null)
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Center(
                    child: GestureDetector(
                      onTap: () {}, // chặn sự kiện tap lan xuống modal
                      child: Material(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (selectedNotification!['rejection_reason'] !=
                                  null)
                                Text(
